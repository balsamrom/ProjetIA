...
def transaction(request):
  """Vue pour afficher la gestion des activités"""
  activities_qs = Activity.objects.all().order_by('activity_name')
  context = {
...
def activity_create(request):
  if request.method == 'POST':
    ...
    if form.is_valid():
      ...
      return redirect('transaction')  # Redirection vers transactions
  else:
    form = ActivityForm()
  return render(request, 'pages/activities/form.html', { 'form': form, 'segment': 'transactions', 'mode': 'create' })
...
def activity_edit(request, pk: int):
  ...
  return render(request, 'pages/activities/form.html', { 'form': form, 'segment': 'transactions', 'mode': 'edit', 'activity': activity })
...
def activity_delete(request, pk: int):
  ...
  if request.method == 'POST':
    ...
    return redirect('transaction')  # Redirection vers transactions
  return render(request, 'pages/activities/confirm_delete.html', { 'segment': 'transactions', 'activity': activity })
